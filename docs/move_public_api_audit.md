...
Issue #105 Step 1 inventory. Scope is `contracts/sources` after moving disaster
registration and affected-cell constructors behind `contracts::accessor`.
...
- `disaster_event`: `create_disaster_registry`,
  `create_from_signed_payload`, `bind_campaign`.
...
- `payout_policy`: `create_default_disaster_policy`,
  ...
  `assert_designated_pool_matches`, `record_claim`.
...
public in Step 1, mostly as admin entrypoints, read accessors, constants, or
assertion helpers. Later steps can decide whether to keep them module-public,
move them behind accessor/view modules, or narrow them.
...
- `affected_cell`: `leaf_hash`, `verify_proof`, `h3_index`, `cell_band`,
  `event_uid`, `event_revision`.
- `claim`: `claim_index_claim_count`, `claim_receipt_summary`,
  `claim_receipt_tier_label`.
- `disaster_event`: `assert_campaign_binding`, `affected_cells_root`,
  `event_uid`, `event_revision`, `occurred_at_ms`,
  `disaster_registry_event_count`, `disaster_event_id`.
- `donation`: `donor_pass_tier_label`, `registry_id`,
  `registry_kind_donor`.
...
- `payload`: `decode_finalized`, `payload_summary`, `event_uid`,
  ...
  `status_finalized`.
- `payout_policy`: `quote_usdc`, `main_backstop_budget_usdc`,
...
No production `public fun` is reclassified to private in Step 1. Existing
