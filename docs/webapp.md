...
- `identifier` は `proof_of_human`（Orb 由来の本人確認）のみ受理する
- `issuer_schema_id` は `1`（Orb の proof_of_human credential）のみ受理する
...
- evidence_hash は v4 の入力（rp_id / environment / action / protocol_version / identifier / nullifier / signal_hash など）から作る
  - `issuer_schema_id` は受理の判定にだけ使い、evidence_hash の入力には含めない
...
- dapp は受け取り操作を `accessor::claim` 1回で実行する。
- 初回 claim では affected cell proof と本人確認情報を渡す。
- 継続 claim では affected cell proof を再取得しない。
- 継続 claim は pass とオンチェーン申請状態から受給可否を判定する。
- 本払いのみの継続 claim では本人確認情報が無くても実行できる。
- dapp merge 前に、`contracts/Published.toml` が `accessor::claim` を含む新 package ID を指していることを確認する。
