...
- `aws:smoke:earthquake-manual` は ManualWatcher Lambda URL に `source_event_id` を POST し、Step Functions execution と DynamoDB row の概要を確認します。出力の `source_archive_summary` で `source_archive_status`、`evidence_manifest_uri`、`evidence_manifest_hash`、`evidence_manifest_artifact_s3_key`、`relayer_mode`、`relayer_digest`、`relayer_object_id`、`disaster_event_object_id`、`floor_census_status`、`floor_census_digest`、`floor_census_counts` を確認します。`disaster_event_object_id` は互換用 alias で、実DB属性は `relayer_object_id` です。relayer submit と Floor Census submit は既存 stack config に依存するため、起動時に明示表示します。
