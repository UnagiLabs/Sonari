...
- local smoke では git 管理外の `dapp/.dev.vars` を使います。
...
4. `dapp/.dev.vars` に `ENOKI_PRIVATE_API_KEY` を入れます。
...
local smoke で同じ key を使う場合は、git 管理外の `dapp/.dev.vars` も更新します。
