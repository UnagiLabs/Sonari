...
Floor Census は earthquake relayer submit 成功後に同じ `RunnerControlLambda` で実行します。`FLOOR_CENSUS_MODE=submit` のときだけ有効で、relayer の既存 Sui Ed25519 鍵 (`RelayerSignerSecretArn`) を census BCS の raw Ed25519 署名と `accessor::set_floor_census` transaction 送信に兼用します。新しい本番鍵は作りません。
...
1. relayer 公開鍵を secret の値からではなく、手元の relayer wallet または既に公開済みの送信者情報から確認する。
2. `metadata_verifier::add_verifier_key` で relayer 公開鍵を `census` family に追加する。
3. `metadata_verifier::create_census_verifier_config` を実行する。
4. `metadata_verifier::register_enclave_instance` を census family / config key に対して実行する。MVP 鍵方式のため PCR は placeholder 値を登録する。これは enclave attestation 方式へ移行するまでの暫定運用です。
5. `metadata_verifier` の config version と registered key が `census_result.move` の `family="census"`, `version=1` と一致することを確認する。
