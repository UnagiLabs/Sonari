...
- `FloorCensusGraphqlUrl=<Sui GraphQL URL>` は任意です。RunnerControlLambda が `CampaignCreated` event を読むための endpoint です。実行時は `FLOOR_CENSUS_GRAPHQL_URL`、`SONARI_SUI_GRAPHQL_URL`、`RelayerNetwork` default の順に選びます。Census TEE 内の count 読み取りは任意 URL を受け取らず、`RelayerNetwork` から canonical Sui GraphQL endpoint を選びます。
