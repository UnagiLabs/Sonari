...
Floor Census は earthquake relayer submit 成功後に同じ runner instance で Census TEE を起動して実行します。`FLOOR_CENSUS_MODE=submit` のときだけ有効です。`RunnerControlLambda` は Census TEE の `/get_attestation` を取得し、`VerifierRegistry` の census config key `3` / family `5` で enclave instance を登録してから、`/process_data` に Floor Census input bundle と registration metadata を渡します。input bundle は event context、`package_id`、`membership_registry_id`、`cell_count_index_id`、`affected_cells` だけを渡し、membership event replay や caller-supplied `counted_cells` は渡しません。Census TEE は bootstrap で受け取った `RelayerNetwork` 相当の `network` から canonical Sui GraphQL endpoint を選び、共有 egress proxy `http://127.0.0.1:18080` 経由で checkpoint、指定された `CellCountIndex` object、そこから辿れる `CellCountShard` dynamic fields、dynamic field count を読みます。`accessor::set_floor_census` transaction の送信には既存 relayer Sui Ed25519 鍵 (`RelayerSignerSecretArn`) を使いますが、census BCS payload の raw Ed25519 署名は Census TEE が行います。
...
- `SonariSuiGraphqlUrl=<Sui GraphQL URL>` は host-side の Sui GraphQL 読み取りを単一の情報源に集約した値です。earthquake watcher の event revision 読み取りと、floor census の event/transaction readback の両方がこの 1 変数（実行時 env 名 `SONARI_SUI_GRAPHQL_URL`）を参照します。未設定時は `RelayerNetwork` から default endpoint を選びます。Census TEE 内の count 読み取りは host から URL を受け取らず、`RelayerNetwork` から canonical Sui GraphQL endpoint を選びます（信頼境界のため固定）。
...
4. smoke 実行時に `RunFloorCensus` が Census TEE の `get_attestation` を登録し、process_data envelope に `registration_metadata.verifier_config_key=3` を渡すことを確認する。
