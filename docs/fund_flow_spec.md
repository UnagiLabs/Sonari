...
- **オンチェーン設計の正は [contracts_spec.md](./contracts_spec.md)** である。
...
| 本払い（Round 1+） | Day 30 / 以後90日ごと | **Campaign のみ** | claim 内 lazy finalize で按分（`min(残高/必要額, 3.0)`） |
...
| G9 | 本払い方式 | 申請＝即時支払い（早い者勝ち。後続ほど減額） | claim 内 lazy finalize で按分 → claim 本払い経路は保存値読むだけ。全員同比率。**Campaign のみ**（補填なし） | 大 |
...
`claim`（受給単一入口・初回申請/本人確認/床払い/本払い/lazy finalize を内包） /
`return_floor_budget` / `sweep_residual` / `spend_operations`）、床払い/本払いの計算式、イベント、定数表は
**[contracts_spec.md](./contracts_spec.md) §5–§13 を正とする**。本書では重複させない。
...
| 現記述 | 改訂案 |
|---|---|
| 「基本の一時支援額は次を目安にする。Band 1: $50 / Band 2: $150 / Band 3: $300」 | 「Band 目標額（保証額ではない）: 50 / 150 / 300 USDC（比率 1:3:6、地域係数なし）。支払いは2段階。**床払い**＝後置センサスで作成直後に `目標額 × min(0.5, 床予算/max_liability)` を固定し検証完了者へ即時。**本払い**＝claim 内 lazy finalize で `目標額 × min(残高/必要額, 3.0)`」へ書き換え |
...
| 現記述 | 改訂案 |
|---|---|
| 「Sponsor donates to Earthquake Pool -> … -> Earthquake Pool pays first -> Main Pool covers allowed shortage -> Relief Receipt is issued」の即時支払いフロー | 2段階フローへ全面書き換え: `災害 finalize（Campaign 自動作成・Category Pool 自動紐付け・同一 tx）→ set_floor_census（登録者数を後置センサスで集計署名→floor_ratio 確定→Category/Main escrow）→ Day 0–30 claim 床払い経路（検証完了しだい固定額即時、Campaign 不使用）→ Day 30 床予算返還＋claim が Round 1 を lazy finalize（Campaign 残のみ按分）→ claim 本払い経路 → 90日ごと再分配 → residual sweep（Main へ）` |
...
| OQ-M5 | `claim`（受給単一入口）の代理実行 | MVP は sender = owner の pull 型。sponsored transaction による push は将来拡張 |
...
Day 0–21     claim（初回）受付: 申請＋本人確認を 1 回で確立
Day 0–30     claim 床払い経路: 検証完了済みへ固定額を随時支払い（Campaign 不使用）
Day 30       return_floor_budget（未消化を Category / Main へ按分返還）
             claim が Round 1 を lazy finalize → 本払い開始（Campaign 残のみ按分）
