...
| `nautilus_disaster_oracle/watcher/` | 軽量監視プロセス。Cron、USGS recent earthquakes API取得、状態管理、Queue投入、TEE起動API呼び出し、手動投入APIを担当する。 | Cloudflare Workers、TypeScript、Wrangler、KV / D1 / Queues |
...
| Oracle Watcher | Cloudflare Workers、Wrangler、KV / D1 / Queues | USGS候補検出、状態管理、TEE起動 |
| Oracle TEE | Rust、Nautilus、Marlin OysterまたはAWS Nitro Enclaves | 外部source再検証、Payload生成、秘密鍵隔離、署名 |
