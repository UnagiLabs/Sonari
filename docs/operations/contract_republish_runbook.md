...
- World ID action は Portal、AWS env、dapp env の 3 面を同じ `sonari_membership_register_v<N>` に揃える。1 面でもずれると proof 検証が通らない。
...
`main` に対象 contract 変更が入っていること、`contracts/Published.toml` の現 package id、GitHub Variables / Secrets の現 object ID、World ID action の現値を控える。`scripts/republish_contracts_bootstrap.ts` を使う場合は、まず dry-run で publish、`GenesisObjectCreated` 解析、`Published.toml` 書換え、GitHub 更新計画を確認する。
...
この変更を `main` に入れると dapp deploy や dev deploy が新 package id を読み始める。GitHub Variables / Secrets の張り替えが終わる前に merge / deploy しない。

### 5. GitHub Variables / Secrets 更新

genesis object と post-publish object を対応表どおり更新する。repo-level Variables と environment Variables が混在するため、`gh variable list --env <env>` と repo-level list の両方で配置先を確認する。

更新後に最低限確認するもの:

- `SONARI_ADMIN_CAP_ID` が新 `AdminCap` を指す。
- `SONARI_VERIFIER_REGISTRY_ID` と `AWS_SONARI_VERIFIER_RUNNER_DEV_RELAYER_VERIFIER_REGISTRY` が新 `VerifierRegistry` を指す。
- `AWS_SONARI_VERIFIER_RUNNER_DEV_RELAYER_REGISTRY` が新 `DisasterRegistry` を指す。
- `SONARI_ALLOWED_RESIDENCE_CELL_REGISTRY_ID` が real root で作成した新 `AllowedResidenceCellRegistry` を指す。
- `SONARI_FLOOR_CENSUS_TARGET` が新 package id を含む `<PACKAGE_ID>::accessor::set_floor_census` である。
- `SONARI_DEV_ADMIN_PRIVATE_KEY` が dev/testnet 用の新 admin key を保持し、ログに出ていない。

### 6. World ID action 更新

World developer portal で新 action を作成し、AWS env と dapp env を同じ値へ更新する。issue #361 の前提では、既存使用済み action が `v1` / `v2` / `v4` / `v5` / `v6` / `v7` で、新規は `sonari_membership_register_v8` とする。
...
| AWS / TEE | `AWS_SONARI_VERIFIER_RUNNER_DEV_WORLD_ID_ACTION` in environment `aws-sonari-verifier-runner-dev` |
| dapp | `NEXT_PUBLIC_WORLD_ID_ACTION` in environment `cloudflare-dapp-worker` |
...
- `SONARI_ADMIN_CAP_ID` が新 `AdminCap`。
- `SONARI_VERIFIER_REGISTRY_ID` と relayer verifier registry が新 `VerifierRegistry`。
- `SONARI_DEV_ADMIN_PRIVATE_KEY` が新 `AdminCap` を操作できる dev/testnet admin key。
- `AWS_SONARI_VERIFIER_RUNNER_DEV_WORLD_ID_ACTION` が Portal / dapp と一致。
...
| `AdminCap` | `GenesisObjectCreated` kind 1 | `SONARI_ADMIN_CAP_ID` |
| `PauseState` | `GenesisObjectCreated` kind 2 | `SONARI_IDENTITY_PAUSE_STATE_ID`, `SONARI_FLOOR_CENSUS_PAUSE_STATE` |
| `MainPool` | `GenesisObjectCreated` kind 3 | `SONARI_FLOOR_CENSUS_MAIN_POOL` |
...
| `MembershipRegistry` | `GenesisObjectCreated` kind 6 | `SONARI_MEMBERSHIP_REGISTRY_ID` |
| `VerifierRegistry` | `GenesisObjectCreated` kind 7 | `SONARI_VERIFIER_REGISTRY_ID`, `AWS_SONARI_VERIFIER_RUNNER_DEV_RELAYER_VERIFIER_REGISTRY` |
| `IdentityRegistry` | `GenesisObjectCreated` kind 9 | `SONARI_IDENTITY_REGISTRY_ID` |
| `CategoryRegistry` | `GenesisObjectCreated` kind 10 | `SONARI_CATEGORY_REGISTRY_ID` |
| `EarthquakePool` | `GenesisObjectCreated` kind 11 | `SONARI_EARTHQUAKE_CATEGORY_POOL_ID`, `SONARI_FLOOR_CENSUS_CATEGORY_POOL` |
| `DisasterRegistry` | `admin::create_disaster_registry` | `AWS_SONARI_VERIFIER_RUNNER_DEV_RELAYER_REGISTRY` |
| `AllowedResidenceCellRegistry` | `admin::create_allowed_residence_cell_registry` | `SONARI_ALLOWED_RESIDENCE_CELL_REGISTRY_ID` |
| floor census target | 新 package id を埋め込む文字列 | `SONARI_FLOOR_CENSUS_TARGET=<PACKAGE_ID>::accessor::set_floor_census` |
...
| World ID action | Portal action bump | `AWS_SONARI_VERIFIER_RUNNER_DEV_WORLD_ID_ACTION`, `NEXT_PUBLIC_WORLD_ID_ACTION` |

## つまりどころ

| 症状 | 典型原因 | 確認 / 対処 |
| --- | --- | --- |
| `EEnclavePcrMismatch` / abort 21 | EIF rebuild 後の PCR0/1/2 と on-chain `VerifierRegistry` が不一致 | dev deploy workflow summary の PCR と on-chain config を比較する。`SONARI_ADMIN_CAP_ID`、`SONARI_VERIFIER_REGISTRY_ID`、`SONARI_DEV_ADMIN_PRIVATE_KEY` が新値か確認して再 deploy する。 |
...
| World ID action mismatch | dapp proof action と TEE `SONARI_WORLD_ID_ACTION` が不一致 | Portal、`AWS_SONARI_VERIFIER_RUNNER_DEV_WORLD_ID_ACTION`、`NEXT_PUBLIC_WORLD_ID_ACTION` が同じ `sonari_membership_register_v<N>` か確認する。 |
...
| old registry への duplicate disaster event | relayer env が古い `DisasterRegistry` を指す | `AWS_SONARI_VERIFIER_RUNNER_DEV_RELAYER_REGISTRY` と stack parameter `RelayerRegistry` が新 `DisasterRegistry` か確認する。 |
...
| `Published.toml` は新しいが dapp / AWS が古い ID を読む | GitHub Variables / Secrets の張り替え漏れ、または deploy 未実行 | `contracts/Published.toml` 由来の package id と各 object ID variable を一覧で照合し、dev deploy / dapp deploy を新 commit で実行する。 |
...
- 対応表の GitHub Variables / Secrets が新 object ID を指す。
...
- World ID action が Portal、AWS env、dapp env で一致。
