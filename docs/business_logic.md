...
`IdentityVerificationRecord.provider_mask` は、本人確認 provider を表す。
...
- AffectedCell の band が `PayoutPolicy.min_claim_band` 以上である。
- IdentityRegistry に有効な本人確認記録がある。
...
finalized time や Sonari の candidate detected time は cutoff に使わない。
...
- Claim 条件へ IdentityRegistry の本人確認記録を追加する。
