...
Web MVP residence signal examples:

- self-declared region
- Verification Fee paid signal
- Passkey / WebAuthn presence
- browser geolocation check count
- check-in day distribution
- IP / VPN / ASN risk bucket
- duplicate email hash signal
- duplicate payment method hash signal
- duplicate passkey / browser fingerprint hash signal
- account age
- wallet / Pass age
...
| high | 強い future signal が整合 | full payout 可能 |

Web-only residence verification should usually produce medium confidence at most. High confidence is reserved for stronger future signals such as mobile attestation, KYC, public digital proof, or trusted institutional records.
