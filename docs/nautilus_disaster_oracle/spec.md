...
| 優先度 | Source | 用途 |
| --: | --- | --- |
| 1 | USGS ShakeMap MMI grid / raster | グローバル標準のセル別MMI判定 |
| 2 | JMA推計震度分布 | 日本国内・日本デモ用の高精度セル別震度判定 |
| 3 | JMA観測点震度 | 補助。面的判定のfallback |
| 4 | USGS event summary `properties.mmi` | fallback。イベント全体の参考値 |
| 5 | Magnitude / 震源情報 | 候補検出・初期テストのみ。finalize根拠にはしない |

日本デモでは、`jma_250m_h3_p90_v1` を優先sourceとして扱えるようにします。
...
```txt
H3 resolution: 7
Cell intensity source: USGS MMI or JMA estimated seismic intensity
Cell aggregation: area_weighted_p90
Cell inclusion: cell_band >= 1
Event band: max(cell_band)
```

中心点サンプリングは境界で誤判定しやすいため非推奨です。平均値は強震域を過小評価しやすく、最大値はノイズや境界誤差に弱いため、標準は `AREA_WEIGHTED_P90` とします。

保守的に広めに拾う運用では、将来 `AREA_WEIGHTED_P95` を追加できます。

ShakeMapやJMA推計震度などのsource自体がまだ取得できない場合、Nautilus / TEEはDisasterEventをfinalizeせず `pending_source` とします。sourceは取得できたがセル単位の揺れ強度がまだ確定できない場合は `pending_mmi` とします。

| Status | 意味 | Claim接続 |
| --- | --- | --- |
| `pending_source` | ShakeMap / JMA推計震度などのsourceがまだ取得できない | 不可 |
...
将来拡張として、新しいsource revisionに置き換えられた過去revisionを `superseded` として扱えます。
...
| `status` | `pending_source` / `pending_mmi` / `finalized` / `rejected` |
...
| `affected_cells_root` | H3 cell、震度値、cell_band、source revisionを含むMerkle root |
| `affected_cells_uri` | `finalized` では必須 |
| `geo_resolution` | 7 |
| `cells_generation_method` | `shakemap_gridxml_h3_p90_v1` / `shakemap_hdf_h3_p90_v1` / `jma_250m_h3_p90_v1` |
| `cell_metric` | `USGS_MMI` または `JMA_SHINDO` |
| `cell_aggregation` | MVP標準は `AREA_WEIGHTED_P90` |
| `intensity_scale` | `MMI_X100` または `JMA_SHINDO_X10` |
| `max_cell_band` | affected cells内の最大Band。`severity_band` と一致 |
| `affected_cell_count` | affected_cellsに含まれるH3セル数 |
...
| Method | 用途 |
| --- | --- |
| `shakemap_gridxml_h3_p90_v1` | MVP標準。USGS ShakeMap `grid.xml.zip` をH3 res7へp90集約 |
| `shakemap_hdf_h3_p90_v1` | production推奨。USGS ShakeMap HDFをH3 res7へp90集約 |
| `jma_250m_h3_p90_v1` | 日本国内・日本デモ用。JMA推計震度分布をH3 res7へp90集約 |

### Merkle leaf要件

`affected_cells_root` は、H3 cellだけでなく震度値とcell_bandを含むleaf集合のrootです。
...
`intensity_value` は `intensity_scale` とセットで解釈します。MMI 7.23は `MMI_X100` で723、JMA震度6弱相当は `JMA_SHINDO_X10` の定義に従います。
...
| affected_cell_count | finalizedでは0より大きいこと |
...
- 状態は `new` / `processing` / `pending_source` / `pending_mmi` / `completed` / `failed` で管理する。
...
- `affected_cells = cell_band >= 1` のH3セル集合。
- `affected_cells_root` はH3 cell、震度値、cell_bandを含むleafから生成する。
...
- JMA震度fixture。
- USGS ShakeMap `grid.xml.zip` を使った `shakemap_gridxml_h3_p90_v1`。
...
- JMA推計震度分布の本番parser。
- 複数sourceのquorum。
- `AREA_WEIGHTED_P95` など集約方式のversion追加。
- UIでH3セルごとのBand可視化。
- Walrus保存。
- 複数Enclave quorum。
...
Sonari Earthquake Oracle v1は、Cloudflare Workerで候補地震を見つけ、Nautilus / TEEでセル単位の揺れ強度を再取得・再検証し、署名付きPayloadとしてSui Moveへ渡す最小構成にします。

Workerは起動・状態管理・見逃し対策に限定し、DisasterEventの最終判定、H3セル別Band判定、Merkle root生成、BCS Payload生成、署名はNautilus / TEEが担います。MoveはWorkerを信頼せず、Nautilus署名Payloadのsignature / status / freshness / revision / source / rootを検証してDisasterEventを作成します。
