...
1. EventBridge Scheduler が AWS Lambda watcher を起動する。
2. AWS Lambda watcher が USGS recent earthquakes API を取得する。
3. AWS Lambda watcher が `type === "earthquake"` の event id を DynamoDB へ冪等記録し、summary fields で軽量 screening する。
4. AWS Lambda watcher が auto-screening を通過した event、または手動投入された event だけを Nautilus runner へ渡す。
...
```mermaid
flowchart TD
  Scheduler[EventBridge Scheduler] --> Watcher[AWS Lambda watcher]
  Manual[Lambda Function URL] --> Watcher
  Watcher --> State[DynamoDB offchain state]
  Watcher --> Workflow[Step Functions Standard]
  Workflow --> Runner[EC2 + Nitro Enclave via SSM]
...
- AWS Lambda watcher は軽量 watcher であり、USGS recent feed の earthquake event id 記録、状態管理、auto-screening、Nautilus 起動だけを担当する。
- Nautilus / TEE は最終判定者として、外部 source を再取得し、Band 判定、H3 生成、Payload 生成、署名を行う。
- `pending_source` / `pending_mmi` / `rejected` / `ignored_small` は offchain DynamoDB state だけで管理し、Sui へ投稿しない。
- Magnitude / summary MMI / alert / tsunami は TEE 起動対象を絞る Watcher 運用フィルタであり、DisasterEvent finalize 条件ではない。
- Relayer は単なる配送係であり、`finalized` Payload だけを投稿する。Move は Relayer や watcher Lambda を信頼しない。
...
| --- | --- |
| MVP 災害 | 地震のみ |
| 候補検出 | AWS Lambda watcher が USGS recent earthquakes API を定期取得し、`type === "earthquake"` を DynamoDB へ記録 |
| Runner 起動フィルタ | AWS Lambda watcher が summary fields で auto-screening し、Step Functions 実行対象を絞る |
| 最終検証 | Nautilus / TEE が USGS 詳細を再取得して実施 |
| セル生成方式 | 初期 MVP から cell_intensity 方式 |
| Band 判定 | H3 セルごとの USGS MMI |
...
| raw data | `raw_data_hash` は必須。Production finalized では `raw_data_manifest.entries[].uri` が Walrus raw source blob を指す |
| TEE | AWS EC2 / Nitro Enclaves 上の Nautilus 実行環境。候補検出時のみ起動する |

Sonari MVP の Disaster Oracle は、USGS ShakeMap が表す地震動だけを claim 対象にする。津波、浸水、火災、原発事故などの二次災害はこの oracle の対象外であり、東日本大震災のような複合災害でも `affected_cells` は USGS ShakeMap の揺れのセルだけを表す。

## 2. 責任分界

### AWS Lambda watcher がやること

- EventBridge Scheduler Trigger で 3〜5 分ごとに起動する。
- USGS recent earthquakes API を取得する。
- 過去 60 分の `type === "earthquake"` event id を重複スキャンする。
- `source_event_id` による冪等管理を行う。
- DynamoDB にイベント状態を保存する。
- USGS recent feed summary fields の `mag`、`mmi`、`alert`、`tsunami` を保存し、auto-screening で `new` と `ignored_small` を分ける。
- threshold 未満の地震は `ignored_small` + `WATCHER_BELOW_AUTO_THRESHOLD` として DynamoDB に残し、runner / TEE には渡さない。
- 後続の USGS summary update で auto-screening 条件を満たした場合のみ、`ignored_small` から `new` へ昇格する。
- auto-screening を通過した地震、または手動投入された地震だけ Step Functions Standard execution を開始する。
- `pending_source` / `pending_mmi` の再チェック予定を管理する。
- watcher Lambda 失敗、AWS runner 起動失敗、timeout、TEE 処理失敗、再試行上限到達を DynamoDB の `error_code` に保存し、admin 通知対象にする。

### AWS Lambda watcher がやらないこと
...
- AWS Lambda watcher から受け取った `source_event_id` をもとに USGS 詳細を再取得する。
...
- raw source data を Walrus に保存し、blob id で再取得・hash 照合したうえで `raw_data_manifest.entries[]` と `raw_data_hash` を生成する。
- ShakeMap grid が `grid.xml.zip` の場合、セル生成には展開後の `grid.xml` を使うが、Walrus archive と `raw_data_manifest.entries[]` の `content_hash` / `source_hash` / `size_bytes` は取得元 URI が指す zip artifact bytes を対象にする。
- Walrus CLI 実行は既定 120,000ms で timeout し、期限超過時は fail-closed にする。`SONARI_WALRUS_CLI_TIMEOUT_MS` または CLI `--walrus-timeout-ms` で正の整数 ms に上書きできる。
...
- DynamoDB へ返す offchain status として `pending_source` / `pending_mmi` / `finalized` / `rejected` を決定する。
...
| Cell Band | USGS MMI | Move での扱い |
| --- | --- | --- |
| Band 0 | MMI VII 未満 | Claim 対象外 |
| Band 1 | MMI VII 以上 | Claim 対象 |
| Band 2 | MMI VIII 以上 | Claim 対象 |
| Band 3 | MMI IX 以上 | Claim 対象 |
...
USGS ShakeMap MMI grid を MVP の唯一の claim source とする。USGS event summary `properties.mmi`、Magnitude、震源情報は Watcher auto-screening と運用上の参考値に限定し、finalize 根拠にはしない。
...
この整理では、Disaster Oracle v1 payload と `AffectedCellLeaf` の BCS field order、USGS 系 numeric enum 値、integer encoding、hash 対象 field、sort rule は変更しない。仕様の正は `schemas/` と golden vector とする。

MVP の impact attestation は Sonari の Claim 判定に使う `affected_cells_root` を固定する。USGS ShakeMap raw source bytes は raw data manifest の hash と Walrus blob id で固定する。

### 入力要件

Nautilus / TEE は、Step Functions / SSM 経由で最低限以下の `DisasterVerifierRequest` を受け取る。
...
AWS Lambda watcher は hash、root、Band、Payload、signature を TEE に渡さない。TEE は source を再取得して自分で生成する。
...
| `raw_data_uri` | raw data manifest の配送先 URI。raw source の canonical reference は `raw_data_manifest.entries[].uri` と `source_hash` |
...
Move は watcher Lambda、Relayer、外部 API レスポンスを直接信頼しない。登録済み Nautilus / TEE 署名 Payload に対して、最低限以下を検証する。
...
| revision | `event_uid` ごとの latest accepted revision を registry に保持し、同一 `(event_uid, event_revision)` 再投稿は duplicate として拒否する。exact duplicate ではない `event_revision <= latest_accepted_revision` は stale revision として拒否し、新しい revision だけを受理する |
...
Move MVP の on-chain v1 は、USGS earthquake + ShakeMap `grid.xml` H3 P90 + USGS MMI だけを受理する。具体的には `primary_source = USGS`、`cells_generation_method = SHAKEMAP_GRIDXML_H3_GRID_POINT_P90_V1`、`cell_metric = USGS_MMI`、`cell_aggregation = GRID_POINT_P90`、`intensity_scale = MMI_X100` を必須にする。

Move 側の DisasterEvent または Registry は、最低限 `event_uid`、`accepted_revision`、`source_updated_at_ms`、`affected_cells_root`、`affected_cells_data_hash` を保持する。受理済み DisasterEvent だけが `DisasterEventCreated` と registry `event_count` の対象になり、古い revision の後出しで claim-facing root を差し戻すことはできない。

Claim 時は、提出された `AffectedCellLeaf` と Merkle proof が `affected_cells_root` に一致することを検証する。Claim 対象セルの `h3_index` は Membership Pass の `verified_residence_cell` と一致し、`cell_band >= min_claim_band` を満たす必要がある。Claim window と residence metadata expiry は Sui `Clock` の `timestamp_ms` で判定し、worker、relayer、caller が渡す任意時刻や `TxContext.epoch_timestamp_ms()` には依存しない。

## 8. 見逃しリスク対策

- Cron は 3〜5 分ごとに再実行する。
- 各 Cron で過去 60 分の USGS recent earthquakes を重複スキャンする。
- `type === "earthquake"` の `source_event_id` は summary auto-screening 結果にかかわらず DynamoDB へ upsert する。
...
- 24 時間未満の地震は finalize せず、DynamoDB の `next_retry_at_ms` で再チェックする。
...
- DynamoDB による primary state 管理。
- Watcher auto-screening による `new` / `ignored_small` 分岐。
- AWS Lambda watcher では finalize せず、Nautilus / TEE が再取得・再検証する責任分界。
...
- Step Functions によるジョブ化。
- 過去 24 時間の定期バックフィル。
- watcher Lambda 失敗・Nautilus 起動失敗通知。
- `cell_metric` / `cell_aggregation` / `intensity_scale` を Payload に含める。
- 異常に大きい `affected_cell_count` の warning log と admin 通知。
- Walrus-backed raw source references in `raw_data_manifest.entries[]`。
...
- DynamoDB upsert 時に summary auto-screening を実行し、`new` / `ignored_small` を分ける。
...
Sonari Disaster Oracle v1 は、AWS Lambda watcher で USGS recent feed の earthquake event id を DynamoDB へ記録し、summary auto-screening で TEE 起動対象を絞り、Nautilus / TEE でセル単位の揺れ強度を再取得・再検証し、`finalized` 署名付き Payload だけを Sui Move へ渡す。
