...
- ShakeMap `grid.xml.zip`、またはJMA推計震度分布を取得・照合する。
...
`event_uid` は同じ地震で固定し、更新は `event_revision` と `source_updated_at_ms` で表します。Move側は同一 `event_uid` で最初に受理したSonari Finalized RevisionをClaim対象にします。古いrevisionと同一revisionの再投稿は拒否し、後続revisionによる差額支払い、減額、返金、top-up claimはMVP対象外です。
...
12. 72時間以内にShakeMap / MMIが取得できなければ、`rejected` + `error_code = REJECTED_AUTO_TRIGGER` にする。
13. `map-status` が `CANCELLED` の場合はfinalize不可とし、`rejected` + `error_code = SHAKEMAP_CANCELLED` にする。
14. Suiへ投稿するのは `finalized` Payloadのみ。

USGSの完全な最終版を待つのではなく、Sonari独自のSonari Finalized Revisionとして確定します。72時間締切を超える再判定やmanual review Objectのオンチェーン投稿はFuture扱いです。
...
MVPのstatusは `new` / `processing` / `pending_source` / `pending_mmi` / `finalized` / `submitted` / `failed` / `rejected` です。AWS runner起動失敗、timeout、TEE処理失敗、Relayer失敗、Move拒否は `error_code` に保存し、admin通知対象にします。
...
| `affected_cells_root` | H3 cell、震度値、cell_band、event_revision、method、oracle_versionを含むMerkle root |
...
- 72時間以内にShakeMap / MMIを取得できなければ `REJECTED_AUTO_TRIGGER` で `rejected` にする。
