...
この文書は `nautilus/verifiers/earthquake/` 配下だけの開発順序を定義する。`contracts/`、`dapp/`、`packages/` の実装は対象外である。ただし、Payload、Merkle leaf、source manifest は root `schemas/` を外部契約として参照する。
...
- 先に pure な Oracle core を作り、AWS Lambda、Step Functions、Nautilus / TEE は後から接続する。
- AWS Lambda watcher は候補検出と状態管理だけを担当し、finalize 判定、H3 生成、hash、BCS、署名は行わない。
...
- `finalized` Payload だけを Relayer に渡す。`pending_source`、`pending_mmi`、`rejected`、`ignored_small` は DynamoDB 状態として扱う。
...
- `nautilus/verifiers/earthquake/shared/`
- `nautilus/verifiers/earthquake/fixtures/`
- `nautilus/verifiers/earthquake/tee/`
- `nautilus/verifiers/earthquake/watcher/`
- `nautilus/verifiers/earthquake/relayer/`
...
- watcher Lambda、TEE core、Relayer の入出力型を分ける。
...
- AWS Lambda watcher から core logic へ信頼済み値を渡さない設計になっている。
- `ignored_small` と `WATCHER_BELOW_AUTO_THRESHOLD` の意味が shared 型・DynamoDB 状態・runner 対象判定で一貫している。
...
- AWS Lambda watcher、Lambda local test、DynamoDB repository を用意する。
- EventBridge Scheduler 相当の scheduled handler で USGS recent earthquakes API を取得する。
- 過去 60 分の `type === "earthquake"` event id を重複スキャンし、`source_event_id` の冪等管理を実装する。
- USGS recent feed summary fields の `mag`、`mmi`、`alert`、`tsunami` を parser / DynamoDB upsert 境界で保持する。
...
- `new`、`processing`、`pending_source`、`pending_mmi`、`finalized`、`submitted`、`failed`、`rejected`、`ignored_small` の遷移を DynamoDB で追跡できる。
- AWS Lambda watcher は finalize 判定や Payload 生成をしていない。
...
- DynamoDB 更新は最初は mock repository でよい。
...
- Node script で固めた orchestration を AWS Lambda watcher local 環境へ接続する。
- local runner adapter 経由で Oracle Core を呼ぶ。
- DynamoDB-compatible repository に status、error_code、retry_count、source_updated_at_ms を保存する。
- finalized の場合だけ Relayer dry-run に渡す。

完了条件:

- Lambda local testで Watcher -> Oracle Core -> Relayer dry-run まで通る。
- DynamoDB-compatible repository に処理履歴と失敗理由が残る。
...
- 署名鍵を TEE 側に隔離する。
- AWS Lambda watcher / Step Functions から受け取る入力は `source_event_id`、`hazard_type`、`primary_source`、`geo_resolution` の最小セットにする。
- TEE 内で USGS detail と ShakeMap を再取得する。

完了条件:

- TEE 外で生成した fixture 結果と、TEE entrypoint 経由の結果が一致する。
- AWS Lambda watcher が渡した hash、root、Band、Payload を信用していない。
- finalized の場合だけ TEE 署名が付く。

### 9. AWS Step Functions runner workflow を接続する
...
- AWS Lambda watcher から Step Functions Standard execution を開始する。
- Step Functions が ASG scale up、instance wait、SSM command、S3 result read、DynamoDB result apply、ASG scale down を段階実行する。
- runner timeout、起動失敗、TEE 失敗を DynamoDB `error_code` に保存する。
...
- finalized 以外は投稿されない。
- DynamoDB の状態だけで処理履歴と失敗理由を追跡できる。
...
- DynamoDB upsert 時に summary auto-screening を実行する。
...
この流れを Node script で安定させてから、Lambda local test local、Nautilus / TEE、AWS runner、Step Functions workflow、live source を順番に接続する。
