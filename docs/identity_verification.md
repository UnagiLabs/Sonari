...
KYC fits the provider extension model, but it is not the live MVP route. The MVP live route is World ID.
...
For World ID, the duplicate key is derived from the World ID context and nullifier. For KYC, the provider can derive it from the provider id and provider user id. For student support, the provider can derive it from a university id, student account id, or enrollment verification id, depending on the source policy.
...
KYC は provider extension model に含まれますが、MVP の live route ではありません。MVP の live route は World ID です。
...
World ID では、World ID context と nullifier から duplicate key を導出します。KYC provider では、provider id と provider user id から導出できます。学生支援では、source policy に応じて university id、student account id、enrollment verification id などから導出できます。
