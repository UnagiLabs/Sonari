...
| 13 | `affected_cell_count` | 影響を受けたH3セルのうち、residence land allowlist に含まれる陸セル数 |
...
- `affected_cells`: affected cells artifact URI、hash、Merkle root、payload count、総セル数、陸セル数、水セル数、H3解像度、residence land allowlist root/version/source hash、分類器名

`affected_cells.root` は ShakeMap から生成した全 affected cells artifact の Merkle root です。`affected_cells.count` と signed payload の `affected_cell_count` は、その中から residence land allowlist に含まれる陸セルだけを数えた値です。水域セルだけの地震は `SEA_ONLY_AFFECTED_CELLS` で `rejected` になり、payload、signature、relayer submit は生成されません。
...
| `SEA_ONLY_AFFECTED_CELLS` | ShakeMap上の affected cells はあるが、residence land allowlist 上の陸セルが0件 |
