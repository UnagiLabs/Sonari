...
| `RELAYER_TARGET` | Relayer preview / dry-run 用 Move target。`<PACKAGE_ID>::accessor::create_disaster_event_and_campaign_from_signed_payload` を設定する |
...
| `RELAYER_CATEGORY_REGISTRY` | Relayer preview / dry-run 用 CategoryRegistry object |
| `RELAYER_CATEGORY_POOL` | Relayer preview / dry-run 用 Earthquake CategoryPool object |
