...
AWS Lambda 上で動く disaster oracle watcher です。USGS recent feed を定期取得し、軽量 screening、DynamoDB 状態管理、Step Functions runner workflow の起動、manual submission を担当します。
...
```txt
pnpm --filter @sonari/oracle-watcher test
pnpm --filter @sonari/oracle-watcher typecheck
```
