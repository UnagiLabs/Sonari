...
### `PAYLOAD_V1_FIELD_ORDER` — Oracleペイロードの26フィールド
...
| 7 | `occurred_at_ms` | 地震発生時刻（ミリ秒） |
| 8 | `observed_at_ms` | データ観測時刻（ミリ秒） |
| 9 | `source_updated_at_ms` | USGSデータの更新時刻（ミリ秒） |
| 10 | `primary_source` | データ提供元（USGS = 1） |
| 11 | `severity_band` | 最大セルバンド（被害の大きさを示す0-3の値） |
| 12 | `source_set_hash` | ソースマニフェストのSHA3-256ハッシュ |
| 13 | `raw_data_hash` | 生データマニフェストのSHA3-256ハッシュ |
| 14 | `raw_data_uri` | 生データの保存場所（Walrus URI） |
| 15 | `affected_cells_root` | 影響セル一覧のMerkleルートハッシュ |
| 16 | `affected_cells_uri` | 影響セルデータの保存場所 |
| 17 | `affected_cells_data_hash` | 影響セルデータのSHA3-256ハッシュ |
| 18 | `geo_resolution` | H3セルの解像度（現在は7固定） |
| 19 | `cells_generation_method` | セル生成方法（ShakeMap grid.xml + H3 + P90 = 1） |
| 20 | `cell_metric` | セルの指標（USGS MMI = 1） |
| 21 | `cell_aggregation` | 集計方法（グリッドポイントP90 = 1） |
| 22 | `intensity_scale` | 強度スケール（MMI×100 = 1） |
| 23 | `max_cell_band` | 影響エリア全体の最大バンド |
| 24 | `affected_cell_count` | 影響を受けたH3セルの総数 |
| 25 | `min_claim_band` | 保険申請に必要な最低バンド（現在は1固定） |
| 26 | `freshness_deadline_ms` | このOracleの有効期限（観測時刻 + 一定時間） |
...
min_claim_band: 1    ← 保険申請に必要な最小バンド
```
...
### `validateWorkerToTeeRequest(input)`
...
// 異常（余分なフィールドがある）
validateWorkerToTeeRequest({ source_event_id: "us7000abc1", unknown_field: true, ... })
...
WorkerToTeeRequest         ← Watcher→TEE リクエスト型
