...
ハッシュアルゴリズム:
- `raw_data_hash`: `SHA-256(canonical raw_data_manifest.json bytes)`
- `raw_data_manifest.entries[].content_hash`: `SHA-256(raw source bytes)`
- `raw_data_manifest.entries[].source_hash`: `SHA-256(raw source bytes)`。Production finalized では `uri` は `walrus://blob/<blob_id>`、`walrus_blob_id` は Walrus の content-addressed blob id を入れる。
- `source_set_hash`: `SHA-256(canonical source_manifest.json bytes)`
- `affected_cells_data_hash`: `SHA-256(canonical affected_cells.json bytes)`
- Merkle leaf と内部ノードのハッシュ: `SHA-256`

MVP中の破壊的変更として、`oracle_version = 1` のdigest contractはSHA-256へ再定義されています。pre-SHA-256のv1 fixture、Merkle root、署名は無効です。
...
| `usgs/great_tohoku_2011` | 東日本大震災のUSGS実データ + ShakeMap + グリッドあり | `finalized` | — |
| `usgs/noto_peninsula_2024` | 能登半島地震のUSGS実データ + ShakeMap + グリッドあり | `finalized` | — |
| `usgs/pending_source_no_shakemap` | ShakeMapプロダクトが存在しない | `pending_source` | `SHAKEMAP_PRODUCT_MISSING` |
| `usgs/pending_mmi_empty_grid` | グリッドXMLは存在するが格子点が0件 | `rejected` | `SHAKEMAP_PARSE_FAILED` |
