...
export interface RunnerLifecycleAdapter {
    ...
    process(
        runnerId: string,
        request: EarthquakeVerifierRequest,
        signal?: AbortSignal,
...
export class HttpRunnerAdapter implements RunnerAdapter {
    ...
    async run(request: EarthquakeVerifierRequest): Promise<TeeCoreResult> {
...
const finalizedPayload: EarthquakeOraclePayload = {
    ...
    evidence_manifest_uri: "ipfs://sonari/examples/us7000sonari/evidence_manifest.json",
    evidence_manifest_hash: `0x${"44".repeat(32)}`,
...
export class MockRunnerAdapter implements RunnerAdapter {
    readonly requests: EarthquakeVerifierRequest[] = [];
