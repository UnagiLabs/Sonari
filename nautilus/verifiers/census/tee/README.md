...
- Validate the caller-bound `CellCountIndex` object and resolve needed
  `CellCountShard` objects from its dynamic fields at the census checkpoint.
...
`cell_count_index_id`, and `affected_cells`. It does not accept replayed membership events,
