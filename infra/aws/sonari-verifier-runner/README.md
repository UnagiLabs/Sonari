...
`SOURCE_ARCHIVER_WALRUS_ENV_SECRET_ARN` は SourceArchiver 専用の固定 schema JSON を指します。secret は archiver Lambda だけが読みます。必須 key は `SONARI_WALRUS_CLIENT_CONFIG_YAML` です。任意 key は `SONARI_WALRUS_CONTEXT` です。Sui wallet を使う場合は `SONARI_SUI_WALLET_CONFIG_YAML` と `SONARI_SUI_KEYSTORE_JSON` を必ずセットで入れます。SourceArchiver はこれらの本文を `/tmp` 配下の private file へ書き出し、`walrus json` root の `config`、必要に応じて `context` と `wallet` として渡します。未知 key や片方だけの wallet 設定は fail-closed です。secret 本文、wallet、keystore、private key はログへ出しません。

`SOURCE_ARCHIVER_WALRUS_LAYER_ARN` は Walrus CLI を提供する Lambda layer です。SourceArchiver は wrapper ではなく `/opt/bin/walrus-real` を実行します。TEE と runner EC2 には Walrus wallet、config、store 用 secret を置きません。
