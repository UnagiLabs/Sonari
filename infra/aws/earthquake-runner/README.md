...
開発段階では TEE binary や enclave 起動 command が頻繁に変わるため、AMI に焼き込まず、S3 に保存した artifact を EC2 起動時または deploy pipeline で取得して配置します。これにより、binary 更新のたびに AMI を作り直さずに検証できます。
