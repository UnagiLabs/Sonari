...
```bash
aws s3 cp dist/aws/earthquake-tee-artifact.tar.gz s3://<bucket>/<prefix>/earthquake-tee-artifact.tar.gz
...
aws cloudformation deploy \
  ...
  --parameter-overrides \
    ...
    TeeArtifactS3Key=<prefix>/earthquake-tee-artifact.tar.gz \
...
GitHub Actions の dev deploy workflow は、commit ごとの Lambda artifact と TEE artifact を S3 に upload し、`ScheduleState=DISABLED` のまま CloudFormation stack を更新します。deploy 前に現在の stack から rollback 用 JSON を作り、`earthquake-runner-dev-rollback` artifact として 14 日間保存します。

dev deploy environment には次の GitHub variables を設定します。Sui CLI と Walrus CLI は workflow 内で download し、sha256 を検証してから使います。
...
| `AWS_EARTHQUAKE_RUNNER_DEV_SUI_CLI_URL` | pinned Sui CLI artifact URL |
| `AWS_EARTHQUAKE_RUNNER_DEV_SUI_CLI_SHA256` | Sui CLI artifact の sha256 |
...
rollback する場合は、該当 workflow run から `earthquake-runner-dev-rollback` artifact を download し、JSON の `GitCommitSha`、`LambdaCodeS3Key`、`TeeArtifactS3Key`、`TeeArtifactSha256` をまとめて戻します。Do not roll back only artifact keys, leaving `GitCommitSha` stale.

rollback JSON は previous commit/artifact metadata だけを持ちます。現在の stack で使っている `LambdaCodeS3Bucket` / `TeeArtifactS3Bucket` は環境変数または現行 stack parameter から渡し、その他の stack parameter は既存値を使います。

rollback JSON に secret ARN、runner token value、keystore path、local file path を追加しないでください。

```bash
STACK_NAME=<dev-stack-name>
ARTIFACT_BUCKET=<current-dev-artifact-bucket>
ROLLBACK_JSON=earthquake-runner-rollback.json

ROLLBACK_GIT_COMMIT_SHA="$(jq -r '.GitCommitSha' "$ROLLBACK_JSON")"
ROLLBACK_LAMBDA_CODE_S3_KEY="$(jq -r '.LambdaCodeS3Key' "$ROLLBACK_JSON")"
ROLLBACK_TEE_ARTIFACT_S3_KEY="$(jq -r '.TeeArtifactS3Key' "$ROLLBACK_JSON")"
ROLLBACK_TEE_ARTIFACT_SHA256="$(jq -r '.TeeArtifactSha256' "$ROLLBACK_JSON")"
...
aws cloudformation deploy \
  ...
  --parameter-overrides \
    ...
    LambdaCodeS3Key="$ROLLBACK_LAMBDA_CODE_S3_KEY" \
    TeeArtifactS3Bucket="$ARTIFACT_BUCKET" \
    TeeArtifactS3Key="$ROLLBACK_TEE_ARTIFACT_S3_KEY" \
    TeeArtifactSha256="$ROLLBACK_TEE_ARTIFACT_SHA256" \
    GitCommitSha="$ROLLBACK_GIT_COMMIT_SHA" \
    ScheduleState=DISABLED \
  --no-fail-on-empty-changeset
```

rollback 後も同じ post-deploy check を実行し、`DeployedGitCommitSha` と artifact key が rollback JSON の値に戻っていること、ASG が `DesiredCapacity=0`、schedule が `DISABLED`、3 つの Lambda の `CodeSha256` が一致していることを確認します。
