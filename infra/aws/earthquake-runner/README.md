...
TEE artifact には Walrus CLI binary も含めるため、dev deploy runner には `walrus` command を用意してください。既定では PATH 上の `walrus` を使います。別 path を使う場合は environment variable / GitHub Actions variable の `AWS_EARTHQUAKE_RUNNER_DEV_WALRUS_CLI` で実行可能 file を指定します。
