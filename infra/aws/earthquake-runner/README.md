...
Sonari 地震オラクルを AWS 内で完結して動かすための CloudFormation テンプレートです。
...
`NitroEnclaveProcessCommand` は、EC2 host 側から Nitro Enclave へ地震 verifier request を渡す command です。SSM 実行時に `SONARI_TEE_SIGNING_KEY_SEED_FILE` から `SONARI_TEE_SIGNING_KEY_SEED` を作成し、TEE process には `SONARI_WALRUS_AGGREGATOR_URL`、`SONARI_WALRUS_CONFIG`、`SONARI_TEE_SIGNING_KEY_SEED` を渡します。

Bootstrap script は `/opt/sonari/runner-token`、`/opt/sonari/tee-signing-key`、`/opt/sonari/walrus-config.json`、`/opt/sonari/runner.env` を `ec2-user:ec2-user` owner、`0400` permission で作成します。
