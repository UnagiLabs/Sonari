# Membership Identity Runner

Legacy dedicated AWS runner stack for membership identity verification (World ID), from artifact build through AWS deploy, VerifierRegistry registration, and Sui submit.
...
# Membership Identity Runner（日本語）

membership identity 検証（World ID）専用の旧 AWS runner stack です。artifact build から AWS deploy、VerifierRegistry registration、Sui submit までを扱います。
