...
| **Membership and identity** | The claimant has an active Membership SBT and a valid World ID verification result. (KYC is a planned future provider, not yet a live claim route.) |
...
All detailed documentation lives under [`docs/`](docs/README.md) — start there for the full reading order.
...
| **membership と identity** | 申請者が有効な Membership SBT と、有効な World ID 検証結果を持つ。（KYC は将来の provider で、現状の MVP では有効な claim 経路ではない。） |
...
詳細なドキュメントはすべて [`docs/`](docs/README.md) 配下にあります。読む順序はそこから始めてください。
