...
The exact amounts, pool math, and timing live in the design spec — see [docs/contracts_spec.md](../docs/contracts_spec.md).
...
In practice, the [`metadata_verifier`](sources/metadata_verifier.move) module is the single on-chain gatekeeper for every signed result. Its extensions, in plain terms:
...
Donations flow into the pools right away. Disaster and identity facts only become real after the **highlighted gate** re-checks their signatures. A survivor's claim then draws on the pools — first the **floor payout** (immediate minimum), and after the donation window closes the **main payout** (a pro-rata share of the campaign). The precise split ratios, escrow rules, and round math are in [docs/contracts_spec.md](../docs/contracts_spec.md).

## Module Map

| Module | What it does |
| --- | --- |
| [`admin`](sources/admin.move) | Genesis setup, the `AdminCap` for least-privilege admin actions, and global / targeted pause |
| [`pools`](sources/pools.move) | The platform-wide `MainPool` (shared relief) and `OperationsPool` (ops fees) |
| [`category_pool`](sources/category_pool.move) | A permanent pool per relief category (e.g. earthquake); the everyday donation home and first source of floor payouts |
| [`campaign`](sources/campaign.move) | The per-disaster object: fundbox, floor escrow, claim applications, and payout rounds |
| [`donation`](sources/donation.move) | Donation intake and atomic split; the donor SBT (`DonorPass`, record only — no claim rights) |
| [`disaster_event`](sources/disaster_event.move) | Creates a `DisasterEvent` from a signed earthquake payload and auto-creates its `Campaign` |
| [`payload`](sources/payload.move) | Decodes and validates the earthquake oracle payload (intent, version, finalized status, freshness) |
| [`census_result`](sources/census_result.move) | Decodes and validates the signed affected-population census used to size the floor payout |
| [`affected_cell`](sources/affected_cell.move) | Merkle-proof check that a claimant's area was actually affected |
| [`allowed_residence_cell`](sources/allowed_residence_cell.move) | Merkle root of the residence-cell allowlist used at registration |
| [`membership`](sources/membership.move) | Issues and manages the `MembershipPass` SBT and the owner's home cell |
| [`identity_registry`](sources/identity_registry.move) | Binds a per-provider duplicate key and stores the identity verification record |
| [`identity_result_v1`](sources/identity_result_v1.move) | Decodes and validates the TEE-signed identity result (KYC / World ID) |
| [`metadata_verifier`](sources/metadata_verifier.move) | The Nautilus gate: enclave keys, PCR configs, expiry, disable controls, and signature verification |
| [`accessor`](sources/accessor.move) | Thin external entry points: version / pause checks, then delegate to package internals |
| [`reader`](sources/reader.move) | Read-only helpers and exported constants for the frontend |
...
- [docs/contracts_spec.md](../docs/contracts_spec.md) — the full design spec: pools, object layouts, exact amounts and constants, security requirements, test requirements, and open questions.
- [nautilus/verifiers/README.md](../nautilus/verifiers/README.md) — the TEE / Nautilus side that *produces* the signed results these contracts verify.
- [docs/donation_flow.md](../docs/donation_flow.md) and [docs/business_logic.md](../docs/business_logic.md) — donor / recipient guide and the business assumptions.
...
正確な金額・Pool の計算・タイミングは設計仕様書にあります — [docs/contracts_spec.md](../docs/contracts_spec.md) を参照してください。
...
実際には、[`metadata_verifier`](sources/metadata_verifier.move) module が、すべての署名済み結果に対する唯一の on-chain ゲートキーパーです。その拡張を平易に説明すると:
...
寄付はすぐ Pool に入ります。災害と本人確認の事実は、**強調されたゲート**が署名を再検証して初めて本物になります。被災者の claim はそこから Pool を引き当てます — まず **床払い**（即時の最低額）、寄付の募集締切後に **本払い**（Campaign の按分の取り分）です。正確な分割比率・escrow ルール・ラウンド計算は [docs/contracts_spec.md](../docs/contracts_spec.md) にあります。

## モジュール一覧

| モジュール | 役割 |
| --- | --- |
| [`admin`](sources/admin.move) | genesis 初期化、最小権限の admin 操作のための `AdminCap`、global / target pause |
| [`pools`](sources/pools.move) | プラットフォーム共通の `MainPool`（共通支援）と `OperationsPool`（運営費） |
| [`category_pool`](sources/category_pool.move) | 支援カテゴリ（例: 地震）ごとの常設 Pool。平常時寄付の受け皿で、床払いの第1資金源 |
| [`campaign`](sources/campaign.move) | 災害ごとのオブジェクト: 募金箱・床予約 escrow・申請・支払いラウンド |
| [`donation`](sources/donation.move) | 寄付の受付と atomic な分割、寄付者 SBT（`DonorPass`、記録のみ・受給権なし） |
| [`disaster_event`](sources/disaster_event.move) | 署名済み地震 payload から `DisasterEvent` を作成し、その `Campaign` を自動作成 |
| [`payload`](sources/payload.move) | 地震 oracle payload の decode と検証（intent・version・finalized status・freshness） |
| [`census_result`](sources/census_result.move) | 床払いの規模を決める、署名済み被災地域センサスの decode と検証 |
| [`affected_cell`](sources/affected_cell.move) | 申請者の地域が実際に被災したことの Merkle proof 検証 |
| [`allowed_residence_cell`](sources/allowed_residence_cell.move) | 登録時に使う許可居住セル allowlist の Merkle root |
| [`membership`](sources/membership.move) | `MembershipPass` SBT と owner の居住セルの発行・管理 |
| [`identity_registry`](sources/identity_registry.move) | provider ごとの duplicate key の束縛と本人確認記録の保存 |
| [`identity_result_v1`](sources/identity_result_v1.move) | TEE 署名済み本人確認結果（KYC / World ID）の decode と検証 |
| [`metadata_verifier`](sources/metadata_verifier.move) | Nautilus ゲート: enclave 鍵・PCR config・有効期限・無効化制御・署名検証 |
| [`accessor`](sources/accessor.move) | 薄い外部エントリー: version / pause チェック後に package 内部へ委譲 |
| [`reader`](sources/reader.move) | frontend 向けの読み取り専用ヘルパーと公開定数 |
...
- [docs/contracts_spec.md](../docs/contracts_spec.md) — 完全な設計仕様: Pool・オブジェクト設計・正確な金額と定数・セキュリティ要件・テスト要件・Open Questions。
- [nautilus/verifiers/README.md](../nautilus/verifiers/README.md) — これらのコントラクトが検証する署名済み結果を *作る* 側（TEE / Nautilus）。
- [docs/donation_flow.md](../docs/donation_flow.md) と [docs/business_logic.md](../docs/business_logic.md) — 寄付者 / 受給者向けガイドと事業前提。
