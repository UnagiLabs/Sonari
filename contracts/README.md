...
> **注意（旧文書との関係）**: [docs/fund_flow_spec.md](../docs/fund_flow_spec.md) は本書より前の
...
| 締切後 | **本払い（Round 1 以降）** | Day 30 に Round 1、以後90日ごと | **Campaign Pool のみ**（Category / Main は使わない） | finalize_round で按分確定。全員同比率 |
...
4. **finalize と claim の分離**: 本払い額はラウンド finalize で一度だけ計算して保存する。
   床払い額はセンサス確定時に固定する。claim は保存値を読むだけで、
   生のプール残高から金額を導出しない。
...
| `floor` | （`campaign` 内に統合可） | センサス受理・floor_ratio 確定・床予算 escrow・claim_floor・床予算返還 |
| `payout` | `PayoutReceipt`, `FloorReceipt` | ラウンド finalize・本払い按分計算・claim_payout・sweep |
...
| `MembershipPass` / `DonorPass` / `PayoutReceipt` / `FloorReceipt` | owned（owner へ transfer、`has key` only） | 不要 |
...
### 7.3 claim_floor（床払いの受取、Day 0–30・本人確認完了しだい随時）
...
- 事前条件 / assert:
  - version / pause / `campaign.paused == false`、`census_set == true`、`floor_budget_returned == false`。
  - SBT precheck（active / sender = owner / registry 整合）。
  - 申請 `ClaimApplication` が存在し、`verified == true` / `excluded == false` / `floor_claimed == false`。
    （本人確認は §9.2 `verify_claim` で完了済み。未検証なら床払いは受けられない。）
...
- 処理: `campaign.floor_balance` の残額を **escrow 元の按分**
  ...
  `floor_budget_returned = true`。以後 `claim_floor` は不可（受取期限到来）。
...
## 9. 本払い（Round 1 以降）Claim / Payout フロー
...
Day 0–30     床払い: verify_claim 済みの受給者へ claim_floor で随時支払い
Day 30       return_floor_budget（未消化床予算を Category / Main へ返還）
             finalize_round (Round 1) → claim_payout 開始（Campaign 残高のみ）
以後90日ごと  finalize_round (Round 2, 3, ...): Campaign 残高を再分配（補填なし）
...
finalize_round（ラウンドごとに1回だけ実行。資金源は Campaign Pool のみ）:
...
- verify 完了で**床払い（claim_floor）と本払いの両方の資格**が立つ。
- 申請受付は Day 21 で締切済みのため、verify が増えても申請者集合は増えない。
  Round N の finalize 後に verified になった申請者は Round N+1 から本払いに参加する
  （床払いは検証完了時点で `floor_budget_returned == false` の間に受け取れる）。

### 9.3 finalize_round（本払いラウンド確定、誰でも実行可）
...
- 資金源は **Campaign Pool のみ**。Category / Main は引数に取らない（本払いで補填しない）。
...
- 資金源は **Campaign Pool のみ**（Category / Main を引数に取らない）。
- assert: version / pause / `campaign.paused == false`、`current_round >= 1`、
  SBT precheck（sender = owner、active）、
  申請が `verified == true` / `excluded == false`、
  `verified_in_round < current_round`（finalize 前に検証済みだった者のみ。`ERoundNotEligible`）、
...
- 事前条件: 終了判定（§9.3）を満たす、または `liability == 0` のままラウンド間隔が経過。
  `closed == false`、`floor_budget_returned == true`（床予算返還後）。
...
| `FloorPaid` | claim_floor | pass_lineage_id、band、amount、recipient |
...
| `RoundFinalized` | finalize_round | round、liability、campaign_available、band_payout[]、eligible_count |
| `PayoutClaimed` | claim_payout | round、pass_lineage_id、band、amount、recipient |
...
| claim_floor | 未センサス reject、未検証 reject、二重受取 reject、固定額どおりの支払い、返還後 reject、owner 宛 transfer、予算非枯渇 |
| return_floor_budget | Day 30 前 reject、按分返還（Category : Main）、端数処理、二重返還 reject、返還後 claim_floor 不可 |
...
| finalize_round | `min(base, 3.0)`、補填なし（Category/Main 不要）、liability=0、実行時刻 guard、Round 2 以降の再分配 |
| claim_payout | 保存値どおりの支払い（Campaign のみ）、round 単位二重受取 reject、verified_in_round guard、excluded reject、owner 宛 transfer |
...
- **床払い `claim_floor`（Day 0–30、固定額）/ `return_floor_budget`（Day 30）**
- `submit_claim` / `verify_claim`（申請と本人確認の分離）
- 本払い `finalize_round` / `claim_payout`（Campaign のみ、Round 1 + 90日ごと再分配）
...
| OQ-8 | 検証遅延者のラウンド参加の状態遷移 | `verify_claim` 時の `current_round` を `verified_in_round` に記録し、`claim_payout` は `verified_in_round < current_round` を要求。finalize は実行時点の `verified_count_by_band` を母数にするため整合 |
...
| [docs/donation_flow.md](../docs/donation_flow.md) | 寄付者・受給者向けの公開ガイド（床払い = 最低ラインの支援金 / 本払い = 寄付の分配） |
| [docs/business_logic.md](../docs/business_logic.md) | 事業・資金設計メモ（§3 Claim 条件・§6 duplicate key は本仕様の前提。§4/§8/§9 は旧設計で本書が正） |
| [docs/fund_flow_spec.md](../docs/fund_flow_spec.md) | 資金フロー設計の旧イテレーション。支払いモデルは本書が正（要再整合） |
