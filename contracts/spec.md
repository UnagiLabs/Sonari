...
PR3 / MVP の donation、pool、donor、budget、payout accounting は USDC 固定で扱う。金額は USDC の最小単位で保存し、USDC decimals は 6 を前提にする。mainnet USDC type / address はこの repository ではまだ固定していないため、PR3 の contracts では `contracts::mock_usdc::USDC` を package-local USDC stand-in として使い、mainnet asset binding は後続 PR で差し替える。任意の `Coin<T>` を受け取る generic donation entry、multi-coin accounting、価格換算、swap、asset whitelist、coin type ごとの Pool 分離は MVP では実装しない。
...
PR3 で実装する Pool はすべて USDC 専用である。`MainPool`、`DesignatedPool`、`OperationsPool` は `Balance<mock_usdc::USDC>` と累計 USDC 受入額を保持し、USDC 以外の `Coin<T>` を deposit できる public API surface を持たない。Designated Donation の split は `main = amount / 2`、`designated = amount - main` とし、奇数額の端数は Designated / Campaign Pool 側へ寄せる。
...
Donation entry は USDC 専用の `donate_general_usdc`、`donate_designated_usdc`、`donate_operations_usdc` として提供する。既存 `DonorPass` を更新する entry も `Coin<mock_usdc::USDC>` 固定とし、任意の `Coin<T>` generic donation は提供しない。zero amount は fail-closed で abort する。donation 前に global pause と対象 Pool pause を検証する。
...
| `donate_general_usdc` | `Coin<mock_usdc::USDC>` を 100% Main Pool に入金し、初回寄付なら DonorPass を発行する |
| `donate_general_usdc_with_pass` | 既存 DonorPass を registry と照合し、USDC DonationRecord と donor 集計を更新する |
| `donate_designated_usdc` | `Coin<mock_usdc::USDC>` を Designated / Campaign Pool と Main Pool に 50/50 split し、初回 DonorPass / DonationRecord / donor 集計を更新する |
| `donate_operations_usdc` | `Coin<mock_usdc::USDC>` を 100% Operations Pool に入金し、初回 DonorPass / DonationRecord / donor 集計を更新する |
...
PR3 は USDC fixed accounting の Pool / Donation / DonorPass 基盤を実装する。実コード API と Move type は追加するが、schema、BCS field order、`AffectedCellLeaf` canonical order、golden vector、Disaster Oracle v1 payload は変更しない。multi-coin 対応、価格換算、swap、asset whitelist、coin type ごとの Pool 分離、mainnet USDC type binding、Campaign Pool の本実装、Claim / Payout 実行は follow-up とする。
