...
#[test]
fun registers_enclave_instance_from_verified_nitro_attestation() {
    let mut scenario = initialized();
    ...
    scenario.next_tx(ADMIN);
    {
        ...
        metadata_verifier::register_enclave_instance_for_testing(
            &mut registry,
            document,
        ...
        assert!(enabled);
        assert!(registered_at_ms == scenario.ctx().epoch_timestamp_ms());
        assert!(expires_at_ms == nitro_fixture_expires_at_ms());
        assert!(disabled_at_ms.is_none());
    ...
    let events = event::events_by_type<metadata_verifier::EnclaveInstanceRegistered>();
    assert!(events.length() == 1);
    let (
        registry_id,
        family,
        ...
        expires_at_ms,
        actor,
    ) = metadata_verifier::enclave_instance_registered_event_fields(*events.borrow(0));
    assert!(registry_id != object::id_from_address(@0x0));
    assert!(family == reader::verifier_family_earthquake_oracle());
    assert!(verifier_version == reader::verifier_version_v1());
    assert!(config_version == 1);
...
fun enclave_registration_rejects_pcr_mismatch() {
    let mut scenario = initialized();
...
fun enclave_registration_rejects_disabled_config() {
    let mut scenario = initialized();
...
fun enclave_registration_rejects_expired_instance_expiry() {
    let mut scenario = initialized();
...
#[test, expected_failure(abort_code = metadata_verifier::EEnclaveInstanceAlreadyRegistered)]
fun enclave_registration_rejects_duplicate_public_key() {
    let mut scenario = initialized();
    ...
    scenario.next_tx(ADMIN);
    {
        ...
        let duplicate_document = nitro_attestation::load_nitro_attestation(
            nitro_attestation_payload(),
            &attestation_clock,
        );
...
#[test, expected_failure(abort_code = metadata_verifier::EEnclaveAttestationMissingPublicKey)]
fun enclave_registration_rejects_attestation_without_public_key() {
    let mut scenario = initialized();
    ...
    scenario.next_tx(ADMIN);
    {
        ...
        let mut registry = scenario.take_shared<metadata_verifier::VerifierRegistry>();
        let mut attestation_clock = clock::create_for_testing(scenario.ctx());
        attestation_clock.set_for_testing(no_public_key_fixture_timestamp_ms());
        ...
        let document = nitro_attestation::load_nitro_attestation(
            no_public_key_attestation_payload(),
            &attestation_clock,
        );
...
#[test]
fun disabling_enclave_instance_flips_enabled_false() {
    let mut scenario = initialized();
    ...
    scenario.next_tx(ADMIN);
    {
        ...
        let (_, _, _, _, enabled, _, expires_at_ms, disabled_at_ms) =
            metadata_verifier::enclave_instance_fields_for_testing(
                &registry,
                nitro_fixture_public_key(),
            );
        assert!(!enabled);
        assert!(expires_at_ms == nitro_fixture_expires_at_ms());
        assert!(disabled_at_ms == option::some(scenario.ctx().epoch_timestamp_ms()));
    ...
    let events = event::events_by_type<metadata_verifier::EnclaveInstanceDisabled>();
    assert!(events.length() == 1);
    let (registry_id, public_key, actor) =
        metadata_verifier::enclave_instance_disabled_event_fields(*events.borrow(0));
    assert!(registry_id != object::id_from_address(@0x0));
    assert!(public_key == nitro_fixture_public_key());
    assert!(actor == ADMIN);
...
fun nitro_fixture_timestamp_ms(): u64 {
    1747071568899
}

fun nitro_fixture_expires_at_ms(): u64 {
    1747075168899
}

fun nitro_fixture_public_key(): vector<u8> {
    x"e8e62201dbe293b703c759f653107acbc2c911fa1d2e66f2c747bec95971a2af"
}

fun nitro_fixture_pcr0(): vector<u8> {
    x"cbe1afb6ed0ff89f10295af0b802247ec5670da8f886e71a4226373b032c322f4e42c9c98288e7211682b258684505a2"
}

fun nitro_fixture_pcr1(): vector<u8> {
    nitro_fixture_pcr0()
}

fun nitro_fixture_pcr2(): vector<u8> {
    x"21b9efbc184807662e966d34f390821309eeac6802309798826296bf3e8bec7c10edb30948c90ba67310f7b964fc500a"
}

fun no_public_key_fixture_timestamp_ms(): u64 {
    1731627987382
}

fun nitro_attestation_payload(): vector<u8> {
...
fun no_public_key_attestation_payload(): vector<u8> {
