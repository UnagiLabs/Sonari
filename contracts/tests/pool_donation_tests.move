...
fun admin_can_create_usdc_pools() {
    ...
    let (main_pool_id, designated_pool_id, operations_pool_id) = create_all_pools(&mut scenario);

    let pool_events = event::events_by_type<pools::PoolCreated>();
    assert!(pool_events.length() == 3);
    ...
    let (event_pool_id, pool_kind, related_id, created_at_ms, actor) =
        pools::pool_created_event_fields(*pool_events.borrow(2));
...
fun general_donation_moves_all_usdc_to_main_pool_and_mints_donor_pass() {
    ...
    scenario.next_tx(DONOR);
    {
        ...
        donation::donate_general_usdc(&pause_state, &mut main_pool, coin, scenario.ctx());
