...
fun target_leaf_hash_and_merkle_proof_match_fixture_vectors() {
    ...
    let hash = allowed_residence_cell::leaf_hash(&leaf);
    assert!(
    ...
    assert!(allowed_residence_cell::verify_proof(&leaf, proof, merkle_root()));
}
...
fun expected_root_must_be_32_bytes() {
    ...
    allowed_residence_cell::verify_proof(&leaf, vector[], vector[0]);
}

#[test, expected_failure(abort_code = allowed_residence_cell::EInvalidHashLength)]
fun sibling_hash_must_be_32_bytes() {
    allowed_residence_cell::new_proof_step_right(vector[0]);
}
