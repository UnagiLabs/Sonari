...
3. 初回寄付時の `DonorPass` issue を実装する。
4. 2 回目以降の `DonationRecord` dynamic field 追加を実装する。
5. donor aggregate と tier 更新を実装する。
6. donation / donor events を実装する。
7. paused donation reject と semi-SBT transfer reject を接続する。
...
3. issue entry と `MembershipPassIssued` event を実装する。
...
- Nautilus 署名付き migration result で owner / payout address を移行できる。
- old owner binding mismatch は reject される。
- new owner binding mismatch は reject される。
- migration 後も `pass_lineage_id` は維持される。
- old pass status は migrated になる。
- migration result なしの owner 変更は reject される。
- 二重 migration は reject される。
- `PassMigrated` が期待フィールドで emit される。

実装順:

1. migration result の verifier family / intent / signature / expiry validation を実装する。
2. old owner / new owner / pass lineage binding を検証する。
3. owner / payout address update を実装する。
4. migrated status と duplicate migration guard を実装する。
5. migration event を emit する。
...
- claimant が `MembershipPass.owner` でも許可済み payout address でもない Claim は reject される。
...
2. MembershipPass validation と required metadata validation を接続する。
3. claimant と owner / approved payout address の一致を検証する。
...
- `git diff --check`、Move build / test、関連 `pnpm` checks を実行していること
