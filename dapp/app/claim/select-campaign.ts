...
/**
 * campaigns から disasterEventId 一致の ClaimCampaignState を返す純粋関数。
 * 見つからなければ null。
 *
 * - 副作用なし・決定的。
 ...
 */
export function selectCampaignByEventId(
    campaigns: readonly ClaimCampaignState[],
    eventId: string,
): ClaimCampaignState | null {
    return campaigns.find((c) => c.disasterEventId === eventId) ?? null;
}
