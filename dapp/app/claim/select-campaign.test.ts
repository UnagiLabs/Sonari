...
describe("selectCampaignById", () => {
    it("一致する disasterEventId が1件のとき、その ClaimCampaignState を返す", () => {
        const result = selectCampaignById([STATE_A], STATE_A.disasterEventId);
    ...
    it("既存 /claim/<campaignId> の互換として campaignId でも返す", () => {
        const result = selectCampaignById([STATE_A], STATE_A.campaignId);
        expect(result).toBe(STATE_A);
    });

    it("一致しない id なら null を返す", () => {
    ...
    it("複数 campaign の中から正しい1件を返す", () => {
        ...
        expect(selectCampaignById(campaigns, STATE_B.disasterEventId)).toBe(STATE_B);
        expect(selectCampaignById(campaigns, STATE_C.disasterEventId)).toBe(STATE_C);
    });
});
