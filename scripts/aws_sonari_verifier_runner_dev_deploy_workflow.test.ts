...
describe("AWS Sonari verifier runner dev deploy workflow", () => {
    ...
    it("uploads commit-scoped artifacts under one Sonari verifier runner prefix", async () => {
        ...
        expectContainsAll(workflow, [
            ...
            `census_tee_key="${s3Prefix}/${githubSha}/census-tee-artifact.tar.gz"`,
            `census_eif_key="${s3Prefix}/${githubSha}/census-tee.eif"`,
            ...
            "s3://$ARTIFACT_BUCKET/$census_tee_key",
            "s3://$ARTIFACT_BUCKET/$census_eif_key",
        ]);
    });
    ...
    it("checks post-deploy guardrails before completing", async () => {
        ...
        expectContainsAll(workflow, [
            ...
            "TeeEifS3KeyOutput",
            "TeeEifSha256Output",
            "CensusTeeArtifactS3KeyOutput",
            "CensusTeeArtifactSha256Output",
            "CensusTeeEifS3KeyOutput",
            "CensusTeeEifSha256Output",
            ...
            `[[ "$census_tee_artifact_s3_key" == "sonari-verifier-runner/${githubSha}/census-tee-artifact.tar.gz" ]]`,
            `[[ "$census_eif_s3_key" == "sonari-verifier-runner/${githubSha}/census-tee.eif" ]]`,
            '[[ "$deployed_census_tee_artifact_sha256" == "$CENSUS_TEE_ARTIFACT_SHA256" ]]',
            '[[ "$deployed_census_eif_sha256" == "$CENSUS_EIF_SHA256" ]]',
        ]);
    });
