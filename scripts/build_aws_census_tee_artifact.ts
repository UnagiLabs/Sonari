...
const DEFAULT_OUTPUT_PATH = "dist/aws/census-tee-artifact.tar.gz";
const DEFAULT_WORK_DIR = ".build/aws-census-tee-artifact";
const CARGO_MANIFEST_PATH = "nautilus/verifiers/census/tee/Cargo.toml";
const CARGO_TARGET_DIR = "target";
const DEFAULT_CARGO_TARGET = "x86_64-unknown-linux-musl";

export interface BuildAwsCensusTeeArtifactOptions {
    outPath?: string;
    keepWorkDir?: boolean;
}

interface ParsedArgs extends Required<BuildAwsCensusTeeArtifactOptions> {}

export async function buildAwsCensusTeeArtifact(
    options: BuildAwsCensusTeeArtifactOptions = {},
    ...
    const cargoTarget = process.env.SONARI_CENSUS_TEE_CARGO_TARGET ?? DEFAULT_CARGO_TARGET;
    const targetBinary = path.resolve(
        process.env.SONARI_CENSUS_TEE_BINARY ??
            path.join(CARGO_TARGET_DIR, cargoTarget, "release/census-tee"),
    );
    const targetBridgeBinary = path.resolve(
        process.env.SONARI_CENSUS_VSOCK_TCP_BRIDGE_BINARY ??
            path.join(CARGO_TARGET_DIR, cargoTarget, "release/vsock-tcp-bridge"),
    );
    const artifactBinary = path.join(workDir, "bin/census-tee");
    ...
    if (process.env.SONARI_CENSUS_TEE_BINARY === undefined) {
        await run("cargo", [
            "build",
            "--release",
            "--target",
...
if (import.meta.url === mainPath) {
    buildAwsCensusTeeArtifact(parseArgs(process.argv.slice(2)))
