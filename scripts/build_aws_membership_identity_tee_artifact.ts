...
const DEFAULT_OUTPUT_PATH = "dist/aws/membership-identity-tee-artifact.tar.gz";
const DEFAULT_WORK_DIR = ".build/aws-membership-identity-tee-artifact";
const CARGO_MANIFEST_PATH = "nautilus/verifiers/membership/tee/Cargo.toml";
const CARGO_TARGET_DIR = "target";
const DEFAULT_CARGO_TARGET = "x86_64-unknown-linux-musl";

export interface BuildAwsMembershipIdentityTeeArtifactOptions {
    outPath?: string;
    keepWorkDir?: boolean;
}

interface ParsedArgs extends Required<BuildAwsMembershipIdentityTeeArtifactOptions> {}

export async function buildAwsMembershipIdentityTeeArtifact(
    options: BuildAwsMembershipIdentityTeeArtifactOptions = {},
    ...
    const cargoTarget =
        process.env.SONARI_MEMBERSHIP_IDENTITY_TEE_CARGO_TARGET ?? DEFAULT_CARGO_TARGET;
    ...
    if (process.env.SONARI_MEMBERSHIP_IDENTITY_TEE_BINARY === undefined) {
        await run("cargo", [
            "build",
            "--release",
            "--target",
...
if (import.meta.url === mainPath) {
    buildAwsMembershipIdentityTeeArtifact(parseArgs(process.argv.slice(2)))
