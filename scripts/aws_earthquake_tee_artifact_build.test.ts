...
describe("AWS earthquake TEE artifact build script", () => {
    it("packages the Walrus CLI and TCP-to-vsock bridge next to the TEE binary", async () => {
        const script = await readFile(scriptPath, "utf8");

        expect(script).toContain('path.join(workDir, "bin/walrus")');
        expect(script).toContain('path.join(workDir, "bin/vsock-tcp-bridge")');
        expect(script).toContain('process.env.SONARI_WALRUS_CLI ?? "walrus"');
        expect(script).toContain("vsock-tcp-bridge");
        ...
        expect(script).toContain("process.env.SONARI_VSOCK_TCP_BRIDGE_BINARY");
