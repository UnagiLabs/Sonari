...
- `aws:smoke:membership-manual` は BatchVerifier Lambda を直接 invoke して queued/retry の identity job を claim させ、応答の `workflow_started` を確認します。続けて Membership runner Step Functions の execution 一覧と最新 execution の input（`verifier_kind=membership_identity`）を確認し、`--job-id` 指定時は対象 job の status を表示します。実行前に想定アカウントと Watcher/Batch schedule の `DISABLED` を確認し、満たさない場合は停止します。job があると Step Functions 経由で runner workflow が動くため、実機実行後は `aws:check-idle` で ASG desired capacity が `0` に戻ったことを確認します。
