...
const DEFAULT_ARTIFACT_PATH = "dist/aws/census-tee-artifact.tar.gz";
const DEFAULT_EIF_PATH = "dist/aws/census-tee.eif";
const DEFAULT_WORK_DIR = ".build/aws-census-eif";
const DEFAULT_DOCKER_URI = "sonari/census-tee:local";
const DEFAULT_CPU_COUNT = 2;
const DEFAULT_MEMORY_MIB = 1024;
const DEFAULT_ENCLAVE_CID = 16;
const CENSUS_TEE_BIN = "/opt/sonari/tee-artifact/bin/census-tee";
const CENSUS_VSOCK_TCP_BRIDGE_BIN = "/opt/sonari/tee-artifact/bin/vsock-tcp-bridge";
const CENSUS_EGRESS_PROXY_PORT = "18080";
const CENSUS_TEE_COMMAND = [
    "/bin/sh",
    "-c",
    `set -e; ip link set lo up || true; ${CENSUS_VSOCK_TCP_BRIDGE_BIN} --listen-host 127.0.0.1 --listen-port ${CENSUS_EGRESS_PROXY_PORT} --parent-cid 3 --vsock-port ${CENSUS_EGRESS_PROXY_PORT} & exec ${CENSUS_TEE_BIN} server`,
] as const;

export interface BuildAwsCensusEifOptions {
    artifactPath?: string;
    eifPath?: string;
    workDir?: string;
    cpuCount?: number;
...
export interface AwsCensusEifBuildPlan {
    artifactPath: string;
    eifPath: string;
...
interface ParsedArgs extends Required<BuildAwsCensusEifOptions> {}

export function createAwsCensusEifBuildPlan(
    options: BuildAwsCensusEifOptions = {},
): AwsCensusEifBuildPlan {
...
export async function buildAwsCensusEif(
    options: BuildAwsCensusEifOptions = {},
): Promise<AwsCensusEifBuildPlan> {
    const plan = createAwsCensusEifBuildPlan(options);
...
if (import.meta.url === mainPath) {
    buildAwsCensusEif(parseArgs(process.argv.slice(2))).catch((error: unknown) => {
