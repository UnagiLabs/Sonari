...
describe("buildSettingsAssignments", () => {
    it("object id 系 GitHub Variables を更新対象にしない", () => {
        const plan = buildSettingsAssignments(fullRewireInput());
        expect(plan.assignments).toEqual([]);
    });

    it("全 genesis object を GitHub Variables ではなく cross-check に分離する", () => {
        const plan = buildSettingsAssignments(fullRewireInput());
        const byKind = new Map(plan.crossChecks.map((c) => [c.objectKind, c.objectId]));
        expect(byKind).toEqual(
            new Map([
                [GENESIS_KIND.ADMIN_CAP, objectId("01")],
                [GENESIS_KIND.PAUSE_STATE, objectId("02")],
                [GENESIS_KIND.MAIN_POOL, objectId("03")],
                [GENESIS_KIND.OPERATIONS_POOL, objectId("04")],
                [GENESIS_KIND.DONOR_REGISTRY, objectId("05")],
                [GENESIS_KIND.MEMBERSHIP_REGISTRY, objectId("06")],
                [GENESIS_KIND.VERIFIER_REGISTRY, objectId("07")],
                [GENESIS_KIND.IDENTITY_REGISTRY, objectId("09")],
                [GENESIS_KIND.CATEGORY_REGISTRY, objectId("10")],
                [GENESIS_KIND.EARTHQUAKE_POOL, objectId("11")],
                [GENESIS_KIND.DISASTER_REGISTRY, objectId("d1")],
                [GENESIS_KIND.ALLOWED_RESIDENCE_CELL_REGISTRY, objectId("e1")],
            ]),
        );
        expect(
    ...
    it("GitHub 設定更新を返さないため secret を扱わない", () => {
        const plan = buildSettingsAssignments(fullRewireInput());
...
describe("runRepublishBootstrap", () => {
    ...
    it("live 実行で publish → residence root update の順に呼び、ID と settings と新 toml を返す", async () => {
        ...
        expect(result.settings?.assignments).toEqual([]);
        expect(
            result.settings?.crossChecks.find((a) => a.objectKind === GENESIS_KIND.ADMIN_CAP)
                ?.objectId,
        ).toBe(objectId("01"));
