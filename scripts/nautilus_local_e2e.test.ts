...
describe("Nautilus local oracle E2E", () => {
    it("finalizes the canonical fixture and builds a relayer request preview", async () => {
    ...
    ] as const)("skips relayer preview for non-finalized case %s", async (caseId, status, errorCode) => {
    ...
    it("keeps every Step 6 fixture eligible under normal watcher screening", async () => {
    ...
    it.each([
        ...
        ["usgs/rejected_cancelled_shakemap", "rejected", "SHAKEMAP_CANCELLED"],
    ] as const)("does not re-run terminal case %s after duplicate scans and process attempts", async (caseId, status, errorCode) => {
