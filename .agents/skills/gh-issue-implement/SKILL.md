...
description: Implement a GitHub issue end to end in this repository using only Codex. Codex owns orchestration, while planning, plan review, step implementation, commit creation, and final review run through dedicated subagents. Use when the user gives a GitHub issue URL or asks to implement, fix, or work on a GitHub issue.
---
...
Codex はオーケストレーターとして進行を管理し、計画・実装・コミット・レビューは専用のサブエージェントへ委譲する。

## 関連サーフェス

- subagents（`.codex/agents/` に定義）:
  - `issue_planner`
  - `plan_reviewer`
  - `issue_step_worker`
  - `commit_worker`（fresh context / `gpt-5.3-codex-spark` / `xhigh` で commit を作成。diff を親に載せない）
...
- supporting skills:
  - `$prepare-issue`
  - `$draft-commit-message`（`commit_worker` がメッセージ規約として読む）
  - `$prepare-pr`
...
7. worktree を作り、step 単位に実装する
8. 各 step は完了直後に `commit_worker` で **その step 専用の 1 commit** を作る
9. ローカル検証を実行
...
- 1 phase / 1 step の小規模 issue で、高リスク surface に触れない場合は、Codex 本体が直接 TDD 実装してよい
...
- fast path の場合も `1 step = 1 commit`、worktree clean、`commit_worker` での commit 作成は維持する
...
6. その step の変更だけを stage し、`commit_worker` で commit して worktree を clean に戻す
...
- **すべての commit は `commit_worker` サブエージェントを必ず使って作る**
- Codex 本体、`issue_step_worker`、その他のサブエージェントが commit message を手書きしてはならない
- `commit_worker` は `gpt-5.3-codex-spark` / `xhigh` / fresh context で起動する
- `commit_worker` は `.agents/skills/draft-commit-message/SKILL.md` を読んでメッセージ規約を適用し、staged diff の確認、メッセージ作成、`git commit` 実行を自身の context 内で完結させる
- `commit_worker` は親と同じ worktree の cwd で動かす。別 worktree や別ディレクトリを作らない
- 親へ返すのは `OK <short-hash> <type>: <title>` または `ERROR: ...` の 1 行だけにする。raw diff を親 context に載せない

コミットメッセージは `$draft-commit-message` の規約に従うが、規約の読み込みと実コミットは `commit_worker` が担当する。
...
- `/review` は通常 1 回だけ実行し、高リスク変更または大きな blocking 修正後だけ最大 1 回再実行する
- 既存の未関連変更は巻き戻さない
...
- step commit、review 対応 commit、finalizer commit を含む **すべての commit** で `commit_worker` サブエージェントを使う
