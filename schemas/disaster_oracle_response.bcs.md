...
`oracle_version = 1` では、以下の26 field orderをimmutable contractとして固定します。field追加、順序変更、型変更、enum値変更が必要な場合は、`oracle_version` をbumpし、新しいschemaとgolden vectorを追加します。既存のv1 payloadを後から読み替えてはいけません。
...
| 24 | `affected_cell_count` | `u64` | `0` より大きいこと。異常値はTEE / Workerでwarning対象 |
