...
このファイルは、repository rootの `schemas/` に置くroot共通仕様です。`AffectedCellLeaf` は、1つのH3セルがfinalized済みのSonari地震イベントのShakeMap affected areaに含まれ、必要なcell band条件を満たすことを証明するleafです。
...
| 8 | `cell_band` | `u8` | affected cell leaf では `1..3`。MMI_X100のthresholdは下記参照 |
...
- `affected_cells_root` は `cell_band >= 1` の affected cells で作ります。ここには residence land allowlist 上の陸セルと、水域と分類された affected cell の両方が含まれます。Band 0セルと全ShakeMap領域の完全証明は含めません。
- signed payload の `affected_cell_count` は、この root に含まれる leaf 総数ではなく、TEE が residence tile manifest / tile shard で分類した陸セル数です。leaf hash と Merkle proof の構造には land/water bit を追加しません。
...
- `cell_band` は `MMI_X100` の値で決定します: `band0` は `< 700`、`band1` は `700..=749`、`band2` は `750..=799`、`band3` は `>= 800` です。`band0` は affected cell leaf / Merkle root には含めません。
