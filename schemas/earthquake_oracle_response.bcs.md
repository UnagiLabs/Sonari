...
| 23 | `max_cell_band` | `u8` | `severity_band` と一致すること。MMI_X100 thresholdは下記参照 |
...
- `severity_band` と `max_cell_band` はClaim対象セルの最大 `cell_band` です。このthreshold変更により、MMI 8.00以上のセルは `band3` になり、v1のfield order、enum値、payload shapeは変更しません。
- `event_uid` は以下で固定します。
