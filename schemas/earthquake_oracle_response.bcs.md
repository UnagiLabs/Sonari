...
| 12 | `affected_cells_root` | `[u8; 32]` | `cell_band >= 1` のShakeMap affected cells を含むMerkle root |
| 13 | `affected_cell_count` | `u64` | `1..1_000_000`。ShakeMap から生成した affected cells のうち、residence land allowlist に含まれる陸セル数 |
...
- `cell_band` は `MMI_X100` の値で決定します: `band0` は `< 700`、`band1` は `700..=749`、`band2` は `750..=799`、`band3` は `>= 800` です。`affected_cells_root` は `band1..band3` の affected cells を含みます。
- `severity_band` は affected cells の最大 `cell_band` です。このthreshold変更により、MMI 8.00以上のセルは `band3` になります。
- `event_uid` は以下で固定します。
...
- `affected_cells_root` は全 affected cells artifact の root です。`affected_cell_count` はその artifact 内の陸セル数であり、`evidence_manifest.affected_cells.land_cell_count` と一致します。`total_cell_count = land_cell_count + water_cell_count` です。
- on-chain Move contract は land/water 分類を再計算しません。TEE が R2 residence tile manifest と tile shard を検証し、陸セル数だけを `affected_cell_count` として署名します。陸セルが0件の場合は `SEA_ONLY_AFFECTED_CELLS` で rejected になり、finalized payload は存在しません。
