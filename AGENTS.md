...
Sui / Walrus を使う開発では、repo 直下の `.local/sonari-dev/` を必ず使用してください。Sui wallet、keystore、Walrus client config、generated aliases、ローカル secret copy はこの配下に置き、infra や package-local な ad hoc path には新規作成しないでください。
